...
const Contact = () => {
  ...
  const formSchema = z.object({
    ...
    message: z.string().min(10, {
      message: t('form.validation.messageRequired'),
    }),
  })
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (result.success) {
        setSubmitStatus({ success: true, message: t('form.success', { default: 'Message sent successfully!' }) })
        form.reset()
      } else {
        setSubmitStatus({ success: false, message: t('form.error', { default: 'Failed to send message. Please try again.' }) })
      }
    } catch (error) {
      setSubmitStatus({ success: false, message: t('form.error', { default: 'Failed to send message. Please try again.' }) })
    } finally {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          <div className="space-y-8">
            <div className="mb-12">
              <h3 className="text-3xl font-bold text-[#302c30] mb-6">{t('contactInfo')}</h3>
              <div className="w-16 h-1 bg-[#9c5748] mb-8"></div>
            </div>

            {contactInfo.map((info, index) => (
              <div
                key={index}
                className="group bg-[#f7f2ee] p-6 transition-all duration-300"
                style={{ clipPath: 'polygon(3% 0, 100% 0, 100% 85%, 97% 100%, 0 100%, 0 15%)' }}
              >
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-[#302c30] rounded-full flex items-center justify-center transition-colors duration-300 flex-shrink-0">
                  ...
                  <div>
                    ...
                    {info.details.map((detail, idx) => (
                      <p key={idx} className="text-[#505248] mb-1">
                        {detail}
                      </p>
                    ))}
                  </div>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 flex-1 flex flex-col">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[#302c30] font-medium">{t('form.email')} {t('form.required')}</FormLabel>
                        <FormControl>
                          <Input
                            type="email"
